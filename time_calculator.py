def add_time(start, duration, *day):
    start_h, start_mm, start_tt = start.replace(':', ' ').split()
    duration_h, duration_m = duration.split(":")
    start_in_mm = int(start_mm) + (int(start_h) * 60)
    duration_in_mm = int(duration_m) + (int(duration_h) * 60)
    total_mm = start_in_mm + duration_in_mm
    ...
    new_m = new_minutes(total_mm)

    def new_period(p):

        if p == 'PM' and (total_mm // 720) % 2 == 0:
            new_per = 'PM'
        elif p == 'AM' and (total_mm // 720) % 2 == 1:
            new_per = 'PM'
        else:
            new_per = 'AM'
        return new_per

    new_p = new_period(start_tt)
    ...
    def count_days(p):
        ...
        if p == 'PM' and int(start_h) == 12:
            ...
            if int(total_mm) < 720:
                ...
                n_day = (total_mm // 1440) + 1
        elif p == 'AM':
            n_day = (total_mm // 1440)

        return n_day

    n_day = count_days(start_tt)
